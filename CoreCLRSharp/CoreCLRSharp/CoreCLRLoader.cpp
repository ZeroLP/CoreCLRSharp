...
#define CoreCLRRuntimePrePathX64 "C:\\Program Files\\dotnet\\shared\\Microsoft.NETCore.App\\"
#define CoreCLRRuntimePrePath "C:\\Program Files (x86)\\dotnet\\shared\\Microsoft.NETCore.App\\"
#define CORECLR_FILE_NAME "coreclr.dll"

CoreCLRLoader* thisHandle;
...
bool CoreCLRLoader::LoadCoreCLRRuntime(LPCSTR runtimeVersion, LPCSTR dllPathDirectory, LPCSTR dllName)
{
    ...
    {
        printf("[CoreCLRLoader]: Failed to load CoreCLR from %s\n", DynamicCLRRuntimePath.c_str());
        return false;
    }
    else
        printf("[CoreCLRLoader]: Loaded CoreCLR from %s\n", DynamicCLRRuntimePath.c_str());
...
bool CoreCLRLoader::InitializeCoreCLRRuntime(LPCSTR dllPathDirectory, LPCSTR dllName)
{
    ...
    {
        printf("[CoreCLRLoader]: coreclr_initialize not found");
        return false;
    }

    if (CreateManagedDelegate == NULL)
    {
        printf("[CoreCLRLoader]: coreclr_create_delegate not found");
        return false;
    }

    if (ShutdownCoreCLR == NULL)
    {
        printf("[CoreCLRLoader]: coreclr_shutdown not found");
        return false;
    }
    ...
    std::cout << "[CoreCLRLoader]: Loading .NET Assembly: " + std::string(dllName) + " from: " + tpaList << std::endl;
    ...
    if (hr >= 0)
        printf("[CoreCLRLoader]: CoreCLR started\n");
    else
    {
        printf("[CoreCLRLoader]: coreclr_initialize failed - status: 0x%08x\n", hr);
        return false;
    }
...
bool CoreCLRLoader::UnloadCoreCLRRuntime()
{
    ...
    if (hr >= 0)
    {
        printf("[CoreCLRLoader]: CoreCLR successfully shutdown\n");
        return true;
    }
    else
        printf("[CoreCLRLoader]: coreclr_shutdown failed - status: 0x%08x\n", hr);
...
bool CoreCLRLoader::InvokeEntryPointMethod(LPCSTR dllNamespace, LPCSTR methodClass, LPCSTR methodName)
{
    ...
    if (hr >= 0)
        printf("[CoreCLRLoader]: Managed delegate created\n");
    else
    {
        printf("[CoreCLRLoader]: coreclr_create_delegate failed - status: 0x%08x\n", hr);
        return false;
    }
